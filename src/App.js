...
import { PricingPage, UpgradeButton } from "./RazorpayPayment";
import {
...
function TrialBanner() {
  ...
  if (days > 7) return null; // only show banner in last 7 days
  return (
    <div style={{background:"#92400e", color:"#fde68a", padding:"8px 20px", fontSize:12, fontWeight:700, textAlign:"center", display:"flex", alignItems:"center", justifyContent:"center", gap:16}}>
      <span>⏰ Your free trial expires in <strong>{days} day{days!==1?"s":""}</strong>.</span>
      <UpgradeButton label="⬆️ Upgrade Now — ₹999/mo" style={{fontSize:11, padding:"4px 12px"}}/>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════
// ██ AUTH GATE  — shown before dashboard
// ═══════════════════════════════════════════════════════════
function AuthGate() {
...
// ═══════════════════════════════════════════════════════════
// ██ TRIAL EXPIRED → shows Razorpay Pricing Page
// ═══════════════════════════════════════════════════════════
// PricingPage is imported from RazorpayPayment.js
// After successful payment it calls refreshUserData() in AuthContext
// which re-checks subscription and routes to Dashboard automatically.

// ═══════════════════════════════════════════════════════════
// ██ MAIN APP  (your original dashboard — role is now
//    "admin" for owner, "staff" for staff members)
// ═══════════════════════════════════════════════════════════
function Dashboard() {
  const { user, shopProfile, signOut } = useAuth();

  // role: owner always gets "admin"
  // When staff login is added this will come from staff_members table
  const role = "admin";
  ...
  const load = async () => {
    ...
    try {
      // 1. Shop info from shop_profiles (already in AuthContext but also load settings)
      ...
      if (settingsData) {
        setShopInfo({
          name:      settingsData.shop_name  || shopProfile?.shop_name || "",
          address:   settingsData.address    || "",
          phone:     settingsData.phone      || "",
          gstin:     settingsData.gst_number || "",
        });
      } else if (shopProfile) {
        setShopInfo({
          ...
          gstin:   shopProfile.gst_number|| "",
        });
      }

      // 2. Medicine master — filter by owner_id (RLS also enforces this)
      ...
      if (mError) { setDbError("Error loading medicines: " + mError.message); }
        ...
        setMasters((mData||[]).map(m => ({
          ...
          sellingRate:  m.selling_price ?? 0,
          purchaseRate: m.purchase_price?? 0,
      ...
      if (sError) { setDbError(prev => prev + " | Sales: " + sError.message); }
        ...
        setSales((sData||[]).map(s => ({
          ...
          gstTotal:        s.gst_amount     ?? 0,
          discount:        s.discount       ?? 0,
          netTotal:        s.net_total      ?? 0,
          notes:           s.notes          || "",
          isReturn:        s.is_return,
          originalInvoice: s.original_invoice || "",
          reason:          s.reason         || "",
          items:           s.items          || [],
        })));
      }
  ...
  const saveMedicine = async () => {
    ...
    try {
      const row = {
        ...
        unit:           mForm.unit,
        supplier_name:  mForm.supplier,
        min_stock:      parseInt(mForm.minStock)    || 10,
  ...
  const savePurchase = async () => {
    ...
    try {
      const row = {
        owner_id:      user.id,
        shop_id:       shopProfile?.id,
        medicine_name: pForm.medicineName,
        batch_no:      pForm.batchNo,
        expiry_date:   pForm.expiryDate,
        supplier_name: pForm.supplierName,
        ...
        notes:         pForm.notes,
        purchase_date: pForm.purchaseDate,
      };
    ...
    try {
      ...
      const { data: savedSale, error: saleErr } = await supabase
        .from("sales").insert({
          ...
          shop_id:        shopProfile?.id,
          invoice_no:     invNo,
          sale_date:      sHeader.saleDate,
          customer_name:  sHeader.customerName  || "",
          customer_phone: sHeader.customerPhone || "",
          ...
          notes:          sHeader.notes || "",
          is_return:      false,
          items:          validItems,           // stored as JSONB
        }).select().single();

      if (saleErr) { setSErr("Error saving sale: " + saleErr.message); setSaving(false); return; }
    ...
    try {
      ...
      const { data: retSale, error } = await supabase
        .from("sales").insert({
          ...
          invoice_no:       retInvNo,
          sale_date:        today(),
          customer_name:    orig.customerName || "",
          net_total:        orig.netTotal,
          subtotal:         orig.subtotal,
          gst_amount:       orig.gstTotal,
          is_return:        true,
          original_invoice: retForm.invoiceNo,
          reason:           retForm.reason,
          items:            orig.items || [],
        }).select().single();

      if (error) { setRetErr("Error: " + error.message); setSaving(false); return; }
      ...
      setSales(prev => [{
        ...
        isReturn: true, originalInvoice: retForm.invoiceNo, reason: retForm.reason,
        items: orig.items||[],
      }, ...prev]);
    ...
    <div style={{minHeight:"100vh", background:"#0f172a", fontFamily:"sans-serif"}}>
      ...
      <div style={{background:"#1e293b", borderBottom:"1px solid #334155", padding:"10px 20px",
        ...
        <div style={{display:"flex", alignItems:"center", gap:10}}>
          ...
          <span style={{color:"#475569", fontSize:11, display:"none"}} className="shop-name">{shopInfo.name}</span>
        </div>
        <div style={{display:"flex", gap:8, alignItems:"center"}}>
          ...
          <span style={{background:"#1e3a5f", color:"#38bdf8", padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:700}}>
            👑 Admin
          </span>
              ...
              <div style={{overflowX:"auto",background:"#1e293b",borderRadius:12,border:"1px solid #334155"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
                  <thead><tr>
                    ...
                    <th style={TH}>Items</th><th style={TH}>Payment</th>
...
function SuperAdminDashboard() {
  ...
  const expiredShops  = shops.filter(s => {
    ...
    if (!sub) return false;
    if (sub.plan === "trial") return new Date(sub.trial_end) < new Date();
    return false;
  }).length;

  // SaaS revenue (paid plans only)
  const totalRevenue = shops
    .filter(s => s.subscriptions?.plan === "paid")
    .length * 999; // ₹999/month assumed
  ...
  return (
    <div style={{minHeight:"100vh", background:"#060d1a", fontFamily:"sans-serif"}}>
      ...
      <div style={{background:"#1e293b", borderBottom:"1px solid #334155", padding:"12px 24px",
        ...
        <div style={{display:"flex", gap:8, alignItems:"center"}}>
          <span style={{background:"#451a03", color:"#f59e0b", padding:"3px 12px", borderRadius:20, fontSize:11, fontWeight:700}}>
            👑 Super Admin
          </span>
          <Btn small onClick={signOut} color="#dc2626">Logout</Btn>
        </div>
      ...
      <div style={{padding:24, maxWidth:1200, margin:"0 auto"}}>
        ...
        {msg && (
          <div style={{background:"#064e3b", color:"#4ade80", padding:"10px 16px", borderRadius:8, marginBottom:16, fontSize:13, fontWeight:600}}>
            ✅ {msg}
          </div>
        )}
        ...
        {activeTab === "shops" && (
          <div style={{background:"#1e293b", borderRadius:12, border:"1px solid #334155", overflowX:"auto"}}>
            <div style={{padding:"14px 20px", borderBottom:"1px solid #334155", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
              ...
              <button onClick={loadData} style={{background:"#334155", color:"#94a3b8", border:"none", borderRadius:6, padding:"5px 12px", cursor:"pointer", fontSize:12}}>
                🔄 Refresh
              </button>
            </div>
            {loading
                      ...
                      {shops.map(shop => {
                        ...
                        return (
                          <tr key={shop.id}>
                            ...
                            <td style={TD}>
                              {sub?.plan === "trial"
                                ? badge("Trial","#92400e","#fbbf24")
                                : sub?.plan === "paid"
                                  ? badge("Paid","#064e3b","#4ade80")
                                  : badge("None","#1e293b","#475569")}
                            </td>
                            <td style={TD}>
                              {isExpired
                                ? badge("Expired","#7c2d12","#f87171")
                                : isActive
                                  ? badge("Active","#064e3b","#4ade80")
                                  : badge("Suspended","#3b0f0f","#f87171")}
                            </td>
                            <td style={{...TD, color: isExpired ? "#f87171" : "#e2e8f0"}}>
                              {sub?.trial_end ? fmt(sub.trial_end) : "-"}
                            </td>
                            <td style={{...TD, color:"#64748b"}}>{fmt(shop.created_at)}</td>
                            <td style={TD}>
                              <Btn small
                                color={isActive ? "#dc2626" : "#059669"}
                                onClick={() => toggleShop(shop.id, shop.owner_id, sub?.status)}>
              ...
              {shops.map(shop => {
                ...
                return (
                  <div key={shop.id} style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px solid #1e293b"}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      {sub?.plan === "paid"
                        ? <span style={{color:"#4ade80", fontWeight:700}}>₹999/mo</span>
                        : <span style={{color:"#f59e0b", fontSize:12}}>Trial</span>}
                    </div>
                  </div>
                );
              })}
...
export default function App() {
  ...
  // 3. Super admin → show super admin dashboard
  if (isSuperAdmin) return <SuperAdminDashboard/>;

  // 4. Trial expired → show Razorpay pricing/payment page
  if (subscription && !isSubscriptionActive()) return <PricingPage/>;

  // 5. Logged in + active → show shop dashboard
  return <Dashboard/>;
}
